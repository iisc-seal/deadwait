    ...
    class FlowInsensitivePurityAnalysis
    {
        ...
        public static PurityAnalysisData MergeStaticConstructorSummaries(Phx.FunctionUnit functionUnit, PurityAnalysisData data)
        {
            ...
            foreach (var typename in staticTypes)
            {
                ...
                var methodsig = "()void";
                var call = new StaticCall(23, 23, 23, null, methodname, typename, methodsig, new List<string>());
                var summary = (new CalleeSummaryReader(functionUnit, moduleunit, true)).GetCalleeData(call, callerData);
                if (summary == null)
                    continue;
