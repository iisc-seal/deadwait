...
namespace SafetyAnalysis.Purity
{
    ...
    {                    
        ...
        public static List<InterestingVariable> Interesting = new List<InterestingVariable>();
        ...
        {            
            ...
            for (uint i = 1; i <= nodeOrder.NodeCount; i++)
            {
                ...
                if (PurityAnalysisPhase.RecordInteresting)
                {
                    RecordInterestingVariables(functionUnit);
                }
        ...
        private void RecordInterestingVariables(Phx.FunctionUnit funit)
        {
            foreach (var inst in funit.Instructions)
            {
                if (inst.IsCallInstruction)
                {
                    List<InterestingVariable> l = GeneralUtil.RecordIfInteresting(inst.AsCallInstruction);
                    if (l != null)
                        Interesting.AddRange(l);
                }
        ...
        private void DumpIR(Phx.FunctionUnit funit)
        {
            ...
            //funit.BuildFlowGraphWithoutEH();
            //if (funit.FlowGraph != null && funit.FunctionSymbol.QualifiedName.Contains("MoveNext"))
            //{
            //    var fg = funit.FlowGraph;
            //    fg.Dump();
            //    fg.DumpDot(PurityAnalysisPhase.outputdir + filename + ".fg.dot", "FlowGraph");
            //}
            writer.Flush();
            writer.Close();
        }
