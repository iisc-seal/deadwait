...
namespace SafetyAnalysis.Purity
{
    ...
    {        
        ...
        public static Dictionary<String, PurityAnalysisData> methodToPurityData = new Dictionary<string, PurityAnalysisData>();        
        ...
        public PurityAnalysisData Execute()
        {
            ...
            if (!endPurityData.OutHeapGraph.IsVerticesEmpty)
            {
                ...
                // reduce
                if (!PurityAnalysisPhase.DisableSummaryReduce && PurityAnalysisPhase.RemoveNonEscaping)
                {
                ...
                //finally reduce
                if (!PurityAnalysisPhase.DisableSummaryReduce && PurityAnalysisPhase.IsFramework == false)
                {
                    if (PurityAnalysisPhase.RemoveNonEscaping && !PurityAnalysisPhase.IsTopDown)
                    {
                        new NodeSkippedCallMerger(
                        LossyNodeMerger.CreateNodeMerger(),
                        CompleteEqualityHash.GetInstance).MergeWithSkippedCalls(endPurityData);
            ...
            if (PurityAnalysisPhase.DumpSummariesAsGraphs)
            {
                ...
                PurityDataUtil.DumpAsDGML(endPurityData, PurityAnalysisPhase.outputdir + filename + ".dgml", null);
        ...
        private void LogPurityData(PurityAnalysisData endPurityData)
        {
            Console.WriteLine("========In Execute======");
            Console.WriteLine("For Method {0}", functionUnit.FunctionSymbol.QualifiedName);
           // Console.WriteLine(endPurityData);
            var filename = GeneralUtil.ConvertToFilename(functionUnit) + ".ohg.summary";
            endPurityData.OutHeapGraph.DumpAsDGML(PurityAnalysisPhase.outputdir + filename + ".dgml");
        }
