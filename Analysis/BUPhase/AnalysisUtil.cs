...
namespace SafetyAnalysis.Purity
{
    ...
    public class AnalysisUtil
    {
        ...
        public static Dictionary<uint, String> idToCall = new Dictionary<uint, string>();
        ...
        public static IEnumerable<HeapVertexBase> GetTargetVertices(DelegateCall dcall,
           PurityAnalysisData data)
        {
            // Console.WriteLine(dcall.GetTarget());
        ...
        {
            // uint id = checked(callInstruction.InstructionId + (callInstruction.FunctionUnit.Number * 100000));
            uint id = callInstruction.InstructionId ^ (callInstruction.FunctionUnit.Number << 13);
            //Console.WriteLine("Obtained id {0} for {1}[{2}][{3}]", id,
            //    callInstruction + " " + callInstruction.FunctionUnit.FunctionSymbol.QualifiedName, callInstruction.FunctionUnit.Number, callInstruction.InstructionId);
            idToCall[id] = callInstruction.ToString() + "[ " + callInstruction.FunctionUnit.FunctionSymbol.QualifiedName + " ]";
            return id;
        }

        public static uint GetCurrentCallId(Phx.IR.Instruction callInstruction)
        {
            uint id = callInstruction.InstructionId;
            //Console.WriteLine("Obtained id {0} for {1}", id,
            //    callInstruction + " " + callInstruction.FunctionUnit.FunctionSymbol.QualifiedName + "[" + callInstruction.InstructionId + "]");
            return id;
        }
        ...
        public static PurityAnalysisData TranslateSummaryToCallerNamespace(
            PurityAnalysisData calleeData, List<object> extStr, List<String> callerNames, uint ictx)
        {
            ...
            ////AS: Vertices in the callee to the cloned vertices in the caller
            //foreach(var entry in nsMap)
            //{
            //    Console.WriteLine("Adding vertex mapping [clone] " + entry.Key + "-->" + entry.Value);
            //    // calleeData.vertexMapping.Add(entry.Key, entry.Value);
            //    PurityAnalysisPhase.vertexMapping.Add(new Pair<String, HeapVertexBase>("", entry.Key), entry.Value);
            //}
            ...
            foreach (var edge in calleeData.OutHeapGraph.Edges)
            {
                ...
                //{
                //    PurityAnalysisData cData;
                //    PurityAnalysisPhase.pointsTo.TryGetValue(callerName, out cData);
                //    if(cData != null)
                //        if (!cData.OutHeapGraph.ContainsHeapEdge(newedge))
                //            cData.OutHeapGraph.AddEdge(newedge.Copy());
                //}
            }
            ...
            AnalysisUtil.TranslateSkippedCalls(calleeData, transData, nsMap, ictx);
        ...
        private static void TranslateSkippedCalls(PurityAnalysisData calleeData, 
            ...
            Dictionary<HeapVertexBase, HeapVertexBase> nsMap, uint directCallerID)
        {
            foreach (var skcall in calleeData.SkippedCalls)
            {
                var transcall = skcall.ShallowClone();
                transcall.resolvingCallInstrID = directCallerID;
