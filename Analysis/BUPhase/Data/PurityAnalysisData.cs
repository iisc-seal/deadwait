...
{    
    ...
    public class PurityAnalysisData : ISerializable
    {
        ...
        // public MultiValueDictionary<HeapVertexBase, HeapVertexBase> vertexMapping;
        ...
        public void InitAuxiliaryState()
        {
            ...
            // this.vertexMapping = new MultiValueDictionary<HeapVertexBase, HeapVertexBase>();
        }
        ...
        public PurityAnalysisData Copy()
        {
            ...
            //if(data.SkippedCalls.Count() > 0 && data.SkippedCalls.ToList()[0].param.Count > 0 && data.SkippedCalls.ToList()[0].param[0].ToString().Contains("CopyToAsync:278"))
            //{
            //    int i = 0;
            //    i++;
            //}
        ...
        private void SanityCheck(PurityAnalysisData data)
        {
            ...
            {
                if (!this.OutHeapGraph.Vertices.Contains(arg))
                {
        ...
        {
            var eargs = from call in this.SkippedCalls
                        from refv in call.GetReferredVertices()
                        select refv;
            foreach (var arg in eargs)
            {
        ...
        public virtual void AddSkippedCall(Call c)
        {
            ...
            //var eargs = from refv in c.GetReferredVertices()
            //            select refv;
            //foreach(var arg in eargs)
            //{
            //    if (!this.OutHeapGraph.Vertices.Contains(arg))
