...
namespace SafetyAnalysis.Purity
{
    ...
    public class PurityAnalysisTransformers
    {
        ...
        {            
            ...
            {                                
                ...
                var variableVertex = NodeEquivalenceRelation.CreateVariableHeapVertex(
                    funcname, NodeEquivalenceRelation.GetVariableId(operand), Context.EmptyContext, operand.Symbol.NameString);
        ...
        private void InitObj(Phx.IR.Instruction instruction, PurityAnalysisData data)
        {
            ...
            var call = new StaticCall(AnalysisUtil.GetCallInstId(instruction), instruction.InstructionId,
                instruction.InstructionId, instruction as Phx.IR.CallInstruction,
                "ctor",declaringtype,"()void",new List<string> { callingMethod });
            var resolver = new CallBackResolver(instruction.FunctionUnit);
            resolver.ApplySummary(call, data, new HigherOrderHeapGraphBuilder(instruction.FunctionUnit), instruction);
        }
