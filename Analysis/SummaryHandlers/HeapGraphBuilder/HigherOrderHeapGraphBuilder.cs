...
namespace SafetyAnalysis.Purity.Summaries
{
    ...
    {
        public HigherOrderHeapGraphBuilder(Phx.FunctionUnit funit, bool trackCallGraph = true)
        {
            functionUnit = funit;
            moduleUnit = funit.ParentPEModuleUnit;
            summaryReader = new CalleeSummaryReader(functionUnit, moduleUnit, trackCallGraph);
        }
            ...
            PurityAnalysisData calleeData)
        {
            ...
            builder.ComposeSummary(callerData,calleeData,initialMapping, call.callingMethodnames);            
        ...
        public bool ComposeResolvableSkippedCalls(PurityAnalysisData data)
        {
            ...
            do
            {
                ...
                foreach (var call in data.SkippedCalls)
                {
                    ...
                    if (callSummary != null)
                    {
                        uint contextid = call.instructionContext;
                        var clonedCalleeData = AnalysisUtil.TranslateSummaryToCallerNamespace(
                                callSummary, new List<object> { contextid }, call.callingMethodnames, call.directCallInstrID);
            ...
            PurityAnalysisData callerData)
        {
            ...
            _newObjectVertex =
                NodeEquivalenceRelation.CreateInternalHeapVertex(
                    scall.methodname, scall.instructionContext, Context.EmptyContext);
