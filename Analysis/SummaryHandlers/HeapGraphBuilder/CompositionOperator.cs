...
{   
    public class CompositionOperator : IDisposable
    {
        ...
        public void ComposeSummary(
            ...
            List<String> callingMethodNames)
        {
            ...
            //{
            //   foreach(var value in finalmap.GetMappedSet(entry.Key))
            //    {
            //        Console.WriteLine("Adding vertex mapping [composition] " + entry.Key + "-->" + value);
            //        // calleeData.vertexMapping.Add(entry.Key, value);
            //        PurityAnalysisPhase.vertexMapping.Add(new Pair<String, HeapVertexBase>("", entry.Key), value);
            //    }
            //}
        }        
        ...
        {            
            foreach (var call in _calleeData.SkippedCalls)
            {
                ...
                if (call is VirtualCall)
                {
                    ...
                    {                        
                        var ncall = vcall.DeepClone() as VirtualCall;
    ...
    public class DoubleCompositionOperator : CompositionOperator
    {
        ...
        {            
            ...
            //    foreach (var value in finalmap.GetMappedSet(entry.Key))
            //    {
            //        Console.WriteLine("Adding vertex mapping [compositionR] " + entry.Key + "-->" + value);
            //        // calleeData.vertexMapping.Add(entry.Key, value);
            //        PurityAnalysisPhase.vertexMapping.Add(new Pair<String, HeapVertexBase>("", entry.Key), value);
            //    }
            //}
