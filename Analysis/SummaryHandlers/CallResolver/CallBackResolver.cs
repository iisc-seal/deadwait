    ...
    {       
            ...
            HeapGraphBuilder builder, Instruction callInstruction)
        {
            ...
            {                
                ...
                ApplyTargetsSummary(call, data, calleeSummary, builder as HigherOrderHeapGraphBuilder, callInstruction);
            }
            else
            {
                ...
                {
                    //PurityDataUtil.DumpAsDGML(data, "Caller"+GeneralUtil.ConvertToFilename(functionUnit), null);
                    var calleeSummary =
                        (new CalleeSummaryReader(functionUnit, moduleUnit, true)).GetCalleeData(call, data);
                    //PurityDataUtil.DumpAsDGML(calleeSummary, "Callee"+GeneralUtil.ConvertToFilename(functionUnit), null);
                    //Console.WriteLine("Target " + functionUnit.FunctionSymbol.QualifiedName);
                    ApplyTargetsSummary(call, data, calleeSummary, builder as HigherOrderHeapGraphBuilder, callInstruction);
                }
            }
        ...
        {            
            ...
            if (call.HasReturnValue())
            {
                ...
                ReturnedValueVertex rv = NodeEquivalenceRelation.CreateReturnedValueVertex(call.instructionContext, Context.EmptyContext);                
