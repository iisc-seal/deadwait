...
namespace SafetyAnalysis.Purity.Summaries
{
    ...
    public class CallSummaryHandler : IPuritySummaryHandler
    {
        ...
        internal override void ApplySummary(Phx.IR.Instruction instruction, PurityAnalysisData data)
        {
            ...
            //Console.WriteLine("Processing call instruction " + callInstruction);
            CallResolver resolver = GetResolver(call, callInstruction.FunctionUnit);                        
            resolver.ApplySummary(call, data, this.GetHeapGraphBuilder(callInstruction), callInstruction);            
        }
        
        public Call GetCallFromInstruction(Phx.IR.CallInstruction callInstruction, PurityAnalysisData data)
        {
            // Console.WriteLine("Processing {0} in {1}", callInstruction, callInstruction.FunctionUnit.FunctionSymbol.QualifiedName);
            uint cid = AnalysisUtil.GetCallInstId(callInstruction);
            uint dcID = callInstruction.InstructionId;
            uint rcID = callInstruction.InstructionId;
            // Console.WriteLine("Assigned cid {0}", cid);
            if (callInstruction.CallTargetOperand != null)
            {
                var x = callInstruction.CallTargetOperand.ToString();
                uint i = 0;
                i++;
            }
            ...
            if (TypeMethodInfoUtil.IsDelegateCall(callInstruction))
            {
                ...
                var dcall = new DelegateCall(cid, rcID, dcID, callInstruction, targetVertex,
            ...
            else
            {
                ...
                if (TypeMethodInfoUtil.IsVirtualCall(callInstruction))
                    call = new VirtualCall(cid, rcID, dcID, callInstruction, PhxUtil.GetFunctionName(funcSym),
                    ...
                    call = new StaticCall(cid, rcID, dcID, callInstruction,
