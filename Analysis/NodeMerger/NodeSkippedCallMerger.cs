...
{    
    ...
    {                                     
        ...
        {            
            ...
            Call newCall = null;
            uint cid = firstcall.instructionContext;
            uint rcID = firstcall.resolvingCallInstrID;
            uint dcID = firstcall.directCallInstrID;
            ...
            if (firstcall is DelegateCall)
            {
                ...
                newCall = new DelegateCall(cid, rcID, dcID, firstcall.directCallInstruction, rep as VariableHeapVertex,(firstcall as DelegateCall).GetSignature(),callingmethods);
            }
            else
            {
                var vcall = firstcall as VirtualCall;
                //create a new virtual call
                newCall = new VirtualCall(cid, rcID, dcID, firstcall.directCallInstruction, vcall.methodname, vcall.signature, vcall.declaringtype, callingmethods);
            }
