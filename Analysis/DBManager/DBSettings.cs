            ...
            get
            {
                ...
                if (!File.Exists(filepath))
                {
                    ...
                    try
                    {
                        ...
                        cmd.CommandText = "CREATE TABLE MethodInfo (typename nvarchar(300) NOT NULL," +
                            "methodname nvarchar(300) NOT NULL,methodSignature nvarchar(4000) NOT NULL," +
                            "IsVirtual bit,IsAbstract bit,IsInstance bit," +
                            "dllname nvarchar(300)," + "ID int IDENTITY," + "PRIMARY KEY (ID))";
                        cmd.ExecuteNonQuery();

                        //create a typeinfo table
                        cmd.CommandText = "CREATE TABLE TypeInfo (typename nvarchar(300)," +
                            "IsInterface bit," + "dllname nvarchar(300)," + "PRIMARY KEY (typename))";
                        cmd.ExecuteNonQuery();

                        //create a typehierarchy table
                        cmd.CommandText = "CREATE TABLE TypeHierarchy (typename nvarchar(300)," +
                            "SuperTypename nvarchar(300)," + "dllname nvarchar(300)," +
                            "PRIMARY KEY (typename,SuperTypename))";
                        cmd.ExecuteNonQuery();

                        //create a puritysummaries table
                        cmd.CommandText = "CREATE TABLE puritysummaries (typename nvarchar(300) NOT NULL," +
                            "methodname nvarchar(300) NOT NULL,methodSignature nvarchar(4000) NOT NULL," +
                            "purityData image," + "dllname nvarchar(300)," +
