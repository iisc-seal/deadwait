...
namespace SafetyAnalysis.Framework.Graphs
{
    ...
    {        
        ...
        public static VariableHeapVertex CreateVariableHeapVertex(string funcname, uint id, Context context, string name = "")
        {                        
            //var funcname = operand.FunctionUnit.FunctionSymbol.QualifiedName;
            VariableHeapVertex varVertex = VariableHeapVertex.New(funcname, id, context, name);
        ...
        //}
        public static uint GetLoadNodeId(Phx.IR.Operand op)
        {
            var inst = op.Instruction;
            if (inst == op.FunctionUnit.DummyInstruction)
                return (uint)(op.ToString().GetHashCode() ^ (inst.FunctionUnit.Number << 13));
            return (inst.InstructionId ^ (inst.FunctionUnit.Number << 13));
        }
