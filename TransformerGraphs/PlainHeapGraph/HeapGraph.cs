...
{    
    ...
    public partial class HeapGraph : HeapGraphBase, ISerializable
    {
        ...
        public HeapGraph()
        {
            ...
            //this.bigraph.VertexRemoved += (HeapVertexBase v) =>
            //{
            //    if (v is VariableHeapVertex)
            //    {
            //        if (v.ToString().Contains("CreateCredentialsFromCache:305:"))
            //        {
            //            Console.WriteLine("Dodgy removal!" + v.ToString());
            //            var t = System.Environment.StackTrace;
            //            Console.WriteLine(t);
            //        }
            //        //throw new Exception("Should not be removing " + v);
            //    }
            //};
        ...
        public override void DumpAsDGML(string fileName)
        {
            GraphUtil.DumpAsDGML<HeapVertexBase, HeapEdgeBase>(
                       fileName,
                       bigraph,
                       (HeapVertexBase node) => ((int)node.Id),
                       (HeapVertexBase node) => ("Type["+ node.GetType().Name +"],Contents["+ node.ToString()+"]"),
                       (HeapEdgeBase edge) => (edge.Field == null ? "" : edge.Field.ToString()),
                       null, null);
        }
